...
- **P3 — Resume tailoring:** ✅ shipped in both surfaces with full parity, including one-click DOCX/PDF export. Web app 2026-07-01 (`api/tailor.js` + `src/ResumeTailor.jsx`, "✨ Tailor Resume" tab). Extension 2026-07-03 (`tailor.html`/`.js`, "✨ Tailor" button per dashboard row, `options.html` for web-app URL + resume text — see `extension/lib/settings.js`). DOCX/PDF export 2026-07-03: web app uses `docx`/`jspdf` (lazy-loaded); extension builds `.docx` by hand (`extension/lib/docx.js` + vendored JSZip) and `.pdf` via `extension/lib/pdf.js` + vendored jsPDF UMD build — one-click, no print dialog, matching the web app exactly. Still open: a "tailor for this job" button on **web app** tracker rows (extension already has it).
